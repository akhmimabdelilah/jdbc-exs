...
public class DevDataService implements IDao<DevData>{
    ...
    public boolean create(DevData o) {
        ...
        try {
            String req = "insert into client values (null,'"+o.getDeveloppeur()+"', '"+o.getJour()+"', '"+sdf.format(o.getNbScripts())+"')";
    ...
    public List<DevData> findAll() {
        ...
        try {
            ...
            rs = st.executeQuery(req);
            while (rs.next()){
              clients.add(new DevData(rs.getInt("id"), rs.getString("nom"), rs.getString("prenom"), rs.getDate("date")));
            }
        } catch (SQLException ex) {
